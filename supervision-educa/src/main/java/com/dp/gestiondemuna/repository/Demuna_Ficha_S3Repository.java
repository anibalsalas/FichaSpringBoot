...
public interface Demuna_Ficha_S3Repository extends JpaRepository<Demuna_Ficha_S3Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s3),0) + 1 from siga.educa_ficha_S3", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S3 f set f.estado_s3 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s3=:id_ficha_s3", nativeQuery = true)
