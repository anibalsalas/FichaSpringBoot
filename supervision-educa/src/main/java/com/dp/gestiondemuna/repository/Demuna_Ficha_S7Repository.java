...
public interface Demuna_Ficha_S7Repository extends JpaRepository<Demuna_Ficha_S7Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s7),0) + 1 from siga.educa_ficha_S7", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S7 f set f.estado_s7 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s7=:id_ficha_s7", nativeQuery = true)
