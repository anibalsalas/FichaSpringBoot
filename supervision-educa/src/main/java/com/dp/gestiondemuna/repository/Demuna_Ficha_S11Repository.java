...
public interface Demuna_Ficha_S11Repository extends JpaRepository<Demuna_Ficha_S11Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s11),0) + 1 from siga.educa_ficha_S11", nativeQuery = true)
...
//    void saveWithoutIdFichaUpdate(@Param("entity") Demuna_Ficha_S11Entity entity);
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S11 f set f.estado_s11 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s11=:id_ficha_s11", nativeQuery = true)
