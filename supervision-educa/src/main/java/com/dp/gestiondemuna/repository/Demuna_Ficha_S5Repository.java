...
public interface Demuna_Ficha_S5Repository extends JpaRepository<Demuna_Ficha_S5Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s5),0) + 1 from siga.educa_ficha_S5", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S5 f set f.estado_s5 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s5=:id_ficha_s5", nativeQuery = true)
