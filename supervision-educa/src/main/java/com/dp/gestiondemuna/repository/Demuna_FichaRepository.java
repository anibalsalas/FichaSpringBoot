...
public interface Demuna_FichaRepository extends JpaRepository<Demuna_FichaEntity, Integer> {
    ...
    @Query("select f from Demuna_FichaEntity f where (trim(f.usu_registro)=trim(:usu_registro)) and  (f.codi_depe_tde=:codi_depe_tde) order by  f.departamento_educa asc,f.provincia_educa asc,f.distrito_educa asc,  f.estado desc")
    ...
    @Query(value = "select nvl(max(id_ficha),0) + 1 from siga.educa_ficha", nativeQuery = true)
...
@Query("SELECT f " +
       ...
       "WHERE (:codi_depa_dpt = '00' OR TRIM(p.des_departament) = TRIM(:codi_depa_dpt)) " +
       "AND (:estado = '00' OR TRIM(f.estado) = TRIM(:estado))")
public List<Demuna_FichaEntity2> listarSeguimientoFichas2(@Param("codi_depa_dpt") String codi_depa_dpt, @Param("estado") String estado);
    ...
    @Query(value = "select upper(trim(w.des_departament)) as region,\n"
            + "0 as comi_comp,\n"
            + "0 as comi_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox) AND  f.estado='C'),0) as dir_comp,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox) AND f.estado='I'),0) as dir_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox) AND  f.estado in ('C','I')),0) as tot_comi_dir,\n"
            + "nvl( (select count(*) from siga.educa_padron wb where trim(wb.coddptox)=trim(w.coddptox) ),0) as tot_cole\n"
    ...
    @Query(value = "select upper(trim(w.des_departament)) as region,\n"
            ...
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado='C'),0) as comi_comp,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado='I'),0) as comi_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado in ('I','C')),0) as total_comi,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado='C'),0) as dir_comp,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado='I'),0) as dir_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado in ('I','C')),0) as total_dir,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where trim(f.codi_depa_dpt)=trim(w.coddptox)  and f.estado in ('I','C')),0) as total_general,\n"
            + "nvl( (select count(*) from siga.educa_padron wb where trim(wb.coddptox)=trim(w.coddptox) ),0) as tot_cole\n"
            + "from siga.educa_padron w group by upper(trim(w.des_departament)), w.coddptox order by w.coddptox asc", nativeQuery = true)
    public List<Object[]> listarCuadroRegistro3();

    @Query(value = "select \n"
            + "nvl( (select count(*) from siga.educa_ficha f where f.codi_depa_dpt is not null and f.estado='C'),0) comi_comp,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where f.codi_depa_dpt is not null  and f.estado='I'),0) comi_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where f.codi_depa_dpt is not null and f.estado='C'),0) dir_comp,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where f.codi_depa_dpt is not null  and f.estado='I'),0) dir_incom,\n"
            + "nvl( (select count(*) from siga.educa_ficha f where f.codi_depa_dpt is not null and  f.estado in ('C','I')),0) tot_comi_dir,\n"
            + "nvl( (select count(*) from siga.educa_padron wb),0) tot_cole\n"
...
//    @Transactional
//    @Modifying
//    @Query(value = "update siga.educa_ficha f set f.estado_s1 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate  where f.id_ficha=:id_ficha", nativeQuery = true)
...
//    @Transactional
//    @Modifying
//    @Query(value = "update siga.educa_ficha f set f.flag_validar=:flag_validar, f.usu_valida=:usu_validar, f.fch_valida=sysdate where f.id_ficha=:id_ficha", nativeQuery = true)
