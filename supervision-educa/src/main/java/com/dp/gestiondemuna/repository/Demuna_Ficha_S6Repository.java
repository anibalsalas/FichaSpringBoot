...
public interface Demuna_Ficha_S6Repository extends JpaRepository<Demuna_Ficha_S6Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s6),0) + 1 from siga.educa_ficha_S6", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S6 f set f.estado_s6 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s6=:id_ficha_s6", nativeQuery = true)
