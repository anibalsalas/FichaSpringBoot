...
public interface Demuna_Ficha_S4Repository extends JpaRepository<Demuna_Ficha_S4Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s4),0) + 1 from siga.educa_ficha_S4", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S4 f set f.estado_s4 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s4=:id_ficha_s4", nativeQuery = true)
