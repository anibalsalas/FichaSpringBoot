...
public interface Demuna_Ficha_S10Repository extends JpaRepository<Demuna_Ficha_S10Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s10),0) + 1 from siga.educa_ficha_S10", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S10 f set f.estado_s10 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s10=:id_ficha_s10", nativeQuery = true)
