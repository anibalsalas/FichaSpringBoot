...
public interface Demuna_Ficha_S9Repository extends JpaRepository<Demuna_Ficha_S9Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s9),0) + 1 from siga.educa_ficha_S9", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S9 f set f.estado_s9 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s9=:id_ficha_s9", nativeQuery = true)
