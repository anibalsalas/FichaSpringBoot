...
public interface Demuna_Ficha_S8Repository extends JpaRepository<Demuna_Ficha_S8Entity, Integer> {
    ...
    @Query(value = "select nvl(max(id_ficha_s8),0) + 1 from siga.educa_ficha_S8", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "update siga.educa_ficha_S8 f set f.estado_s8 = '1', f.usu_valida=:usu_valida, f.fch_valida=sysdate where f.id_ficha=:id_ficha and f.id_ficha_s8=:id_ficha_s8", nativeQuery = true)
